...
```text
Commit này ghi nhận đúng commit docs/test nêu trên, thay cho placeholder cũ.
```
...
Frontend đang set `lastUpdate` bằng thời điểm browser gọi `/api/vcoms/cases` thành công. Đây là thời điểm web refresh, không phải thời điểm Outlook watcher thật sự đọc và rebuild dữ liệu.

Do đó người dùng có thể hiểu nhầm:

```text
"Cập nhật: 10:05:30" = dữ liệu Outlook đã sync lúc 10:05:30
```

trong khi thực tế đó chỉ là thời điểm frontend nhận response từ backend.

### Hướng xử lý dự kiến

Backend `/api/vcoms/cases` cần trả thêm meta:
...
Frontend cần hiển thị tách bạch:
...
### Gợi ý nguồn dữ liệu sync

Ưu tiên đọc từ SQLite vì hiện hệ thống đã có các bảng trạng thái/runs của Outlook reader:
...
Nếu chưa có dữ liệu từ các bảng này, fallback có thể dùng `updated_at` mới nhất trong `vcoms_case_state`, nhưng phải ghi rõ đây là fallback/snapshot time, không phải sync run thật.

Không nên dùng thời điểm frontend gọi API để đại diện cho thời điểm dữ liệu thật.

### Phạm vi cần giữ

```text
- Không đổi logic lọc Kanban.
- Không đổi SLA.
- Không đổi phân quyền.
- Không đổi thống kê.
- Không làm tăng chiều cao header quá nhiều.
- Không đẩy Kanban xuống thấp.
...
[ ] Bước 5: Header/Kanban hiển thị tách biệt thời điểm Web refresh và Dữ liệu sync thực tế.
```
