...
| 4 | Sửa audit/snapshot manual action để undo chính xác | MEDIUM | TODO | Chưa thực hiện. Cần lưu snapshot đầy đủ trước khi manual action và dùng snapshot đó để undo chính xác. |
...
`apply_manual_action()` hiện chỉ lưu snapshot rất hẹp vào audit:

```text
old_value = {"current_stage_code": old[5]}
```

Trong khi `remove_manual_action()` lại cố đọc nhiều field khác từ `old_value` như:
...
Nếu các field này không có, hàm undo dùng default và tự dựng lại `current_stage_label` từ mapping. Điều này có rủi ro trả hồ sơ về trạng thái không đúng 100% so với trước khi thao tác thủ công.

### Mục tiêu

Khi thực hiện manual action:

```text
MANUAL_WAIT_DISBURSE
MANUAL_DONE
```

phải lưu snapshot đầy đủ trạng thái cũ của hồ sơ trước khi update.

Khi hủy thao tác thủ công, phải dùng snapshot mới nhất để restore chính xác trạng thái cũ.

### Field cần lưu/restore tối thiểu
...
Có thể bổ sung field khác nếu trong `vcoms_case_state` có liên quan trực tiếp đến trạng thái hoàn tất/thao tác thủ công.

### Nguyên tắc khi thực hiện

```text
- Không đổi API contract nếu không bắt buộc.
- Không đổi giao diện nếu không cần.
- Không đổi Kanban/SLA/statistics logic ngoài phần restore state của manual action.
- Phải backward-compatible với audit cũ chỉ có current_stage_code.
- Không làm ảnh hưởng manual override cb_httd.
```
