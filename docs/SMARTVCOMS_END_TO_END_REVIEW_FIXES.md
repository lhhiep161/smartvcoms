...
| 2 | Tách API read-only cho room/officer status | HIGH | IMPLEMENTED / NEED CODEX TEST | Đã thêm `GET /api/vcoms/lookup/config`, frontend non-admin dùng `loadLookupConfig()` thay vì phụ thuộc `/admin/config`. Cần Codex build/test. |
| 3 | Tách quyền manual action/manual override | MEDIUM/HIGH | TODO | Chưa thực hiện. |
...
- backend chạy được
- frontend không lỗi build do SmartVCOMSPage.vue / TabStatistics.vue
- GET /api/vcoms/lookup/config trả 401 nếu chưa đăng nhập
- Khi đăng nhập user có quyền SmartVCOMS part ban_dieu_phoi hoặc thong_ke: endpoint lookup/config trả success
- User chỉ có quyền Thống kê vẫn thấy dropdown phòng
- User chỉ có quyền Bàn điều phối vẫn thấy trạng thái cán bộ nếu có cb_config
- User không có quyền Quản trị vẫn không gọi được /api/vcoms/admin/config
- Tab Quản trị và Rule Engine của admin vẫn hoạt động
```
