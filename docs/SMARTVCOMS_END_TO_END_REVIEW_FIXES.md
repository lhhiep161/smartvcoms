...
| 5 | Tách thời điểm web refresh và thời điểm dữ liệu sync thực tế | MEDIUM | DEFERRED / SKIPPED FOR NOW | Bước này chủ yếu làm rõ hiển thị, không ảnh hưởng luồng xử lý SmartVCOMS. Tạm bỏ qua ở vòng hiện tại, không xóa khỏi backlog; sẽ xem xét xử lý sau nếu cần tăng tính minh bạch trạng thái dữ liệu. |
...
Bước này chủ yếu làm rõ hiển thị/truyền thông trạng thái dữ liệu, không thay đổi luồng xử lý chính của SmartVCOMS.

Hiện frontend đang set thời điểm `lastUpdate` bằng thời điểm browser gọi `/api/vcoms/cases` thành công. Đây là thời điểm web refresh, không phải thời điểm Outlook watcher sync dữ liệu thật.

Tuy nhiên, việc hiển thị chưa tách riêng 2 mốc thời gian này không ảnh hưởng đến:
...
Vì vậy, tạm thời bỏ qua Bước 5 trong vòng xử lý hiện tại, không xóa khỏi backlog. Có thể quay lại sau nếu cần tăng tính minh bạch trạng thái dữ liệu cho người dùng.
...
Kanban và Statistics đang lọc dữ liệu "hôm nay" theo ngày server tại thời điểm gọi API:

```text
operational_date = datetime.now().date()
```

Cách này đơn giản nhưng dễ lệch kỳ vọng vận hành nếu sau này cần:

```text
- chạy lại dữ liệu ngày trước,
- debug dữ liệu theo ngày,
- xử lý ca vận hành lệch ngày hệ thống,
- hoặc cho phép chọn ngày vận hành trên Bàn điều phối.
```

### Mục tiêu bước 6

Trước mắt chỉ chuẩn hóa helper xác định ngày vận hành để các service dùng chung, không nhất thiết thêm selector ngày trên UI ngay.

Hướng xử lý dự kiến:

```text
- Thêm helper get_operational_date() hoặc resolve_operational_date().
- Kanban dùng helper thay vì gọi datetime.now().date() trực tiếp.
- Statistics mode=today dùng cùng helper.
- API trả meta/debug nhẹ về operational_date nếu không ảnh hưởng frontend.
- Không đổi business logic SLA, phân quyền, manual action.
```
...
[ ] Bước 6: Chuẩn hóa business date / operational_date.
```
