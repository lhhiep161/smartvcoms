...
| Giảm giãn cách dòng Bàn điều phối | DONE | Đã giảm nhẹ khoảng 10% chiều cao dòng; font bảng từ 15px xuống 14px; chỉ chỉnh CSS trong `SmartVCOMSPage.vue`, không đổi logic dữ liệu. |
...
```text
- Font bảng hồ sơ: 15px -> 14px.
- Padding ô bảng: 8px 6px -> 6px 5px.
- Thêm/đặt line-height khoảng 1.15 cho th/td.
- Padding cột Kanban: 10px 15px -> 9px 13px.
- Header và tiêu đề phụ giảm nhẹ khoảng 10-15%.
```
...
```text
- Chỉ phần Bàn điều phối trong SmartVCOMSPage.vue.
- Các bảng hồ sơ chính/phụ và tiêu đề cột Kanban.
```
...
- API call
- data mapping
- logic phân cột Kanban
```

Lưu ý rebuild/restart:

```text
- Backend: không cần restart.
- Frontend dev/hot reload: chỉ cần refresh.
- Nếu dùng bản build tĩnh: cần rebuild frontend để thấy thay đổi.
```
