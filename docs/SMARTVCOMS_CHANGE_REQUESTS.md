...
| Giảm giãn cách dòng Bàn điều phối | TODO | Chỉ giảm nhẹ khoảng 10% chiều cao dòng; font bảng từ 15px xuống 14px; không đổi logic dữ liệu. |
...
Mục tiêu lần này chỉ giảm nhẹ khoảng 10% chiều cao dòng hồ sơ để vẫn giữ độ dễ đọc. Không thực hiện giảm mạnh.

Yêu cầu cụ thể:

```text
- Font chữ trong bảng hồ sơ giảm từ khoảng 15px xuống 14px.
- Chiều cao dòng hồ sơ giảm khoảng 10% so với hiện tại.
- Chỉ giảm nhẹ padding/margin/line-height, không làm giao diện bị chật hoặc khó đọc.
- Không đổi logic dữ liệu, filter, sort, màu SLA, thao tác click chi tiết, API call, mapping dữ liệu hoặc logic phân cột Kanban.
```
...
Nếu code hiện tại đang dùng giá trị khác 15px/8px, hãy giảm tương ứng khoảng 10%, không ép về mức quá thấp.
...
```text
Mục tiêu chỉnh lần này:
- Chỉ giảm nhẹ khoảng 10% chiều cao dòng hồ sơ.
- Font bảng hồ sơ giảm từ khoảng 15px xuống 14px.
- Màn hình hiển thị được nhiều hồ sơ hơn một chút nhưng vẫn dễ đọc.
...
```text
1. Giảm nhẹ khoảng 10% giãn cách dòng Bàn điều phối.
