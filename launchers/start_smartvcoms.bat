...
start "SmartVCOMS Backend" cmd /k python -m uvicorn backend.main:app --host 0.0.0.0 --port 8000
start "SmartVCOMS Outlook Watcher" cmd /k python backend\modules\smartvcoms\runner\vcoms_today_full_runner.py --watch --interval %VCOMS_WATCH_INTERVAL_SECONDS% --reader-today-only --process-scope today --rebuild-case-state-v2 --db %VCOMS_DB_PATH%
